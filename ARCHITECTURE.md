...
6. Если на последней итерации LLM снова запросила поиск, система выполняет
   его, добавляет фрагменты к накопленным и принудительно просит модель
   сформировать итоговый ответ без инструментов (FINAL_ITERATION_PROMPT).
7. Если LLM не дала ответ (сбой API) - fallback через `_answer_from_chunks()`
